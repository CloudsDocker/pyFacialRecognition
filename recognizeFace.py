import cv2,sys
inputImageFile=sys.argv[1] # the sample image file
faceBase='haarcascade_frontalface_default.xml' # the HAAR cascade file, which contains the machine learned data for face detection
faceClassifier=cv2.CascadeClassifier(faceBase)
objImage=cv2.imread(inputImageFile) # use cv2 to load image file
